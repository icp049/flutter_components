...
class _AlertDetailsPageState extends State<AlertDetailsPage> {
  ...
  void addComment(String alertId, String commentText) async {
    if (commentText.isNotEmpty) {
      try {
        await FirebaseFirestore.instance
            ...
            .add({
          'text': commentText,
          'postedBy': 'User', // Replace 'User' with actual username
   ...
   Future<void> _incrementViewCount() async {
    final alertId = widget.data?['alertId'] ?? '';
    if (alertId.isNotEmpty) {
      try {
        ...
        await FirebaseFirestore.instance.runTransaction((transaction) async {
          ...
          transaction.update(alertRef, {'views': currentViews + 1});
        });
    ...
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        setState(() {
          ...
          _hasMoreComments = querySnapshot.docs.length == commentsPerPage;
        });
      } else {
        setState(() {
